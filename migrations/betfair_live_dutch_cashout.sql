-- ============================================================================
-- betfair_live_dutch_cashout.sql — Fase 5: azioni 'dutch' e 'cashout_all'.
-- Estende la coda betfair_live_order_queue.sql con due NUOVE azioni:
--   action='dutch'       → il worker (_do_dutch) ripartisce uno stake su N selezioni a
--                          PROFITTO UGUALE (trading/dutching, math server-side) e piazza ogni
--                          gamba (stesso path/validazioni del place). params: {selections:
--                          [{selection_id,price[,weight]}], total_stake, side, mode, persistence}.
--   action='cashout_all' → il worker (_do_cashout_all) fa il green-up (flatten) di OGNI
--                          selezione del mercato con esposizione ≠ 0. params: {fraction}.
--
-- side/price/size NON sono dati al top-level per queste azioni (derivati/nei params).
--
-- IDEMPOTENTE: DROP/ADD CONSTRAINT per CONTENUTO (come betfair_live_greenup.sql),
-- CREATE OR REPLACE FUNCTION. Da applicare DOPO betfair_live_order_queue.sql e
-- betfair_live_greenup.sql.
-- ============================================================================

-- 1) Allarga il CHECK 'action' per ammettere 'dutch' e 'cashout_all' (trovando il constraint
--    per CONTENUTO, non per nome — vedi note money-critical in betfair_live_greenup.sql).
BEGIN;
DO $$
DECLARE
    c_name text;
...
ALTER TABLE public.betfair_live_order_requests
    ...
    CHECK (action IN ('place','cancel','replace','place_submin','greenup','dutch','cashout_all'));
COMMIT;


-- 2) request_betfair_live_order — con i rami di validazione per 'dutch' e 'cashout_all'.
...
BEGIN
    ...
    IF v_action IS NULL OR v_action NOT IN
       ('place','cancel','replace','place_submin','greenup','dutch','cashout_all') THEN
    ...
    ELSIF v_action = 'dutch' THEN
        -- dutching: market + params.selections + params.total_stake. side/price/size NON al top.
        ...
        END IF;
    ELSIF v_action = 'cashout_all' THEN
        IF nullif(p->>'market_id','') IS NULL THEN
            RAISE EXCEPTION 'cashout_all: market_id obbligatorio';
        END IF;
        IF (p->'params'->>'fraction') IS NOT NULL
           AND (p->'params'->>'fraction')::numeric NOT BETWEEN 0.0 AND 1.0 THEN
            RAISE EXCEPTION 'cashout_all: params.fraction deve essere in [0.0, 1.0]';
        END IF;
