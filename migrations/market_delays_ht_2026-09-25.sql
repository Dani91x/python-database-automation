-- ============================================================================
-- market_delays_ht_2026-09-25.sql  (ADDITIVA: CREATE OR REPLACE, stessa firma)
-- ============================================================================
-- RITARDO DI MERCATO: UNA SOLA REGOLA, UN SOLO POSTO (reperto R7 dell'audit tab
-- Dashboard del 24/09, AUDIT_2026-09-24/AUDIT_TAB_DASHBOARD_2026-09-24.md).
--
-- COSA CAMBIA rispetto a sql/market_delays_rpc.sql (che resta nel repo come storia):
--   1. REGOLA HT: una partita SENZA dato di primo tempo (halftime_* NULL) NON e'
--      un 0-0. Sui mercati che usano il primo tempo (ovpt, unpt, ggpt, ggst, pf1x,
--      pf2x, pfx1, pfx2, pt1, ptx, pt2) quella partita e' ESCLUSA dalla serie
--      (outcome NULL), esattamente come fa get_market_frequency. Resta contata in
--      meta.n_scope; meta.n_ht_missing dice quante sono; meta.ht_missing_rule =
--      'escluse' dichiara la regola al frontend.
--      Prima: coalesce(halftime,0) -> la partita senza HT contava come 0-0 e
--      gonfiava i ritardi degli Over PT (esempio del referto: lega 547, over 0.5 1T).
--   2. MERCATI FT: INVARIATI riga per riga (stesso scope, stesso ordine, stesse
--      formule): la regola tocca solo i mercati HT.
--   3. MERCATI NUOVI (additivi, servono al cruscotto Direzione che da oggi legge
--      il ritardo da QUI invece di ricalcolarlo nel browser):
--        '1' vittoria casa FT, '2' vittoria trasferta FT, 'gg' entrambe segnano FT,
--        'ng' non entrambe FT, 'unpt' under primo tempo (linea), 'pt1' 'ptx' 'pt2'
--        esito del primo tempo. I 13 mercati del foglio restano identici.
-- Il frontend (lib/signalContext.ts) chiama questa stessa RPC: tab Ritardi e
-- cruscotto Direzione mostrano lo stesso numero.
-- Si applica dal SQL Editor (la applica l'utente). Idempotente.
-- ============================================================================
-- MODULO "STUDIO RITARDI" - RPC di sola lettura
-- Riproduzione del file Excel "STUDIO RITARDI_BASE_v5.0" (con la regola HT sopra).
-- ============================================================================
...
-- IN ('FT','AET','PEN')).
-- COPERTURA HT (dal 25/09): come get_market_frequency, le righe senza primo tempo
-- sono ESCLUSE dalla serie dei mercati HT (non piu' 0-0). La copertura HT resta
-- esposta in meta.ht_coverage_pct, il numero di escluse in meta.n_ht_missing.
--
...
'Studio Ritardi (riproduzione del file Excel; dal 25/09 le partite senza HT sono ESCLUSE dai mercati HT, non contate 0-0; fonte unica del ritardo anche per il cruscotto Direzione): per (lega, mercato, target, intervallo) calcola la macchina W/L->RIT->SUC e tutte le statistiche dei ritardi (frequenza, media storica/quota oggettiva, ritardo attuale, record, media ritardi, distribuzione serie, ultime 10 serie, storico serie %, sotto/sopra media, run sopra-media). Sola lettura su matches, settlement 90 identico a get_market_frequency.';
