...
def delete_existing_top_cards(league_id: int, season_year: int) -> None:
    ...
    try:
        delete_con_ritentativi(
            _delete,
            etichetta=f"delete top_cards league_id={league_id} season_year={season_year}",
        )
    except Exception as e:
        logger.error(
            "❌ Cancellazione top_cards fallita dopo i ritentativi per league_id=%s, season_year=%s: %s",
            league_id,
            season_year,
            e,
        )
