...
export const MIKE_PARAM_FIELDS: readonly MikeParamField[] = [
    ...
    { key: 'pre_max_spread_ticks', label: 'Spread max (tick)', kind: 'number', step: 1, min: 1, max: 10, hint: 'spread back/lay oltre il quale non si entra', group: 'pre' },
    { key: 'pre_green_ticks', label: 'Green-up a (+tick)', kind: 'number', step: 1, min: 1, max: 10, hint: '2 = chiude quando il lay è 2 tick sotto l’ingresso, profitto spalmato', group: 'pre' },
    { key: 'pre_exit_mode', label: 'Chiusura pre-match', kind: 'choice', choices: ['taker', 'resting'], hint: 'taker = al best quando i tick ci sono; resting = ordine appoggiato (solo con coda flumine)', group: 'pre' },
    ...
    { key: 'cover_wait_hazard_max', label: 'Attendi se hazard ≤', kind: 'number', step: 0.01, min: 0, max: 1, hint: 'P(gol nei 3′) dall’Atlante sotto cui si può aspettare', group: 'cover' },
    { key: 'cover_wait_max_min', label: 'Attendi al massimo fino al minuto', kind: 'number', step: 1, min: 0, max: 45, hint: 'poi si copre comunque', group: 'cover' },
...
export const MIKE_PARAM_DEFAULTS: Record<string, number | boolean | string> = {
    ...
    pre_max_spread_ticks: 3, pre_green_ticks: 2, pre_exit_mode: 'taker', pre_entry_ttl_s: 60,
    ...
    cover_enabled: true, cover_profit_factor: 1.2, cover_policy: 'auto', cover_wait_hazard_max: 0.08,
    cover_wait_max_min: 15, cover_wait_p4_max: 0.16, cover_postgoal_delay_s: 45, cover_max_goals: 2,
