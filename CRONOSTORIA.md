...
**h19:10 — ORDINE UTENTE 18:20 (riferito da admin-26)**: «finite tutti i lavori, non c'è bisogno dei replay se stiamo usando i dati live in paper: certificate dai dati che riceviamo le operazioni in paper; alla fine ditemi cosa fare e lo stato attuale; voglio finire entro 2 ore massimo» → scadenza 20:15. admin-26 ha PUSHATO 67c3ad4 (stream stallo/escalation, log figli in `_logs/`, F-9 modo effettivo, scalper freno/origine/force-flat, Mike fill al best, catchup, atlante, frame Mike v4), 0dbc127 (tennis CLOSED/tetto/Terminal), 78d9f65 (K1 GBP→EUR). Pull ff-only ok (master 78d9f65). Piano: consegna dei miei 3 cantieri entro 19:30 (solo pezzi certificati, patch + referto), integrazione 19:30-19:40, UN riavvio dell'app alle 19:40 (admin-26), riavvio 3 in paper dal mio delegato, osservazione 19:45-20:05, consolidato 20:10. Freno 2° ciclo: aperture RIPRESE dopo il rilascio (Safe 354, tennis_swing 3/2, scalper: le 2 sessioni armate a freno tirato sono partite = effetto R-F2-10).
**h19:25 — SINTESI FINALE FASE 2 (delegato, 16:55Z)**: 50 controlli · OK 34 (68 %) · KO 7 (14 %) · NC 9 (18 %); P&L netto paper regolato: Omega +8,55 (9/0), Mike +2,90 (3/3), Safe esatto +7,60 (4/0), Safe tennis −2,76, tennis_swing +0,15, tennis_pro −0,12, tennis_flb −0,04. Nuovo R-F2-20: Safe tennis via canale → `tennis_live_orders.source='manual'`. Mike 5070-5075 tutti regolati. Riavvio 3 pronto (`accensione_r3`), registratore fermato alle 19:35. Commit su master con file espliciti (no jsonl).
**h20:00 — TRE CANTIERI INTEGRATI SU MASTER (verificati da me: patch puliti, tsc 0, test campione, una mutazione mia per cantiere)**: FIX-B c4a5f30 (47 file: Analytics/Ritardi < 8 s con riepiloghi + passo non-fatale nel job, Direzione quota Betfair/«quota book», ML verdetto unico, TacticAI esito reale, rese, kickoff da matches.fixture_date; mutazione golPrevisto → 1 rosso); FIX-C 351121a (13 file: CustodeSessione in auth.py = causa radice della cecità = sessione Betfair scaduta nel buco di rete; scanner rientro stream + avvisi; Safe reconcile safe_tennis-t; scalper avvio/orfane; spec watchdog; mutazione mia sul margine 90 % SOPRAVVISSUTA → test chiesto al delegato, fix_c2); FIX-A 46a3f39 (27 file: Safe/Omega per modalità, Live P&L, /mike uscite, totali, conteggi, tentativi Omega non consumati da freno/aggancio; mutazione kill_switch → 1 rosso; NON FATTO R8 FOK Omega paper: decisione utente). admin-26 ha pushato b0fe3b1 (motore ordini). Riavvio app alle ~19:50-20:00 da admin-26 con npm run build; poi riavvio 3 in paper.
**h19:10 (orologio PC; i miei orari precedenti di oggi erano avanti di ~40 min) — TUTTI I FIX SU MASTER E APP RIAVVIATA**. Miei, ognuno
verificato da me (test su master, falsificazioni rieseguite, mutazione mia): `b0fe3b1` motore ordini (lato normalizzato → journal e
coda ok; source = nome del bot calcio/tennis con `migrations/betfair_live_orders_source_bot_2026-09-26.sql` DA APPLICARE e riscrittura
senza source se il CHECK rifiuta; R11 ref univoci fra avvii; in_aggancio 10 s; canale_fase): 38 test, mutazione «lato grezzo» 4 rossi;
`07bc872`+`73c1046` UI pagine (F-1..F-8, F-10, F-12, F-13 + paper/live separati: tick col segno giusto, P&L per bot, un solo giorno
del denaro = regolamento, proposte scadute decadute dopo 12 h, netto di commissione, Market Watch «FINITA», set_summary completo):
pytest 222, tsc 0, vitest 10 file/186; `2220288` custode della sessione Betfair (FIX-C di B, causa radice della cecità: sessione .it
scaduta nel buco di rete con keepAlive che ingoiava l'errore) cablato nei runner calcio (relogin → alert + ricostruzione con guardie)
e tennis (relogin → stall_worker): 4 test. Sessione B: `c4a5f30` FIX-B (Analytics/Ritardi < 8 s, Direzione, ML, TacticAI actual,
rese), `351121a` FIX-C (CustodeSessione, scanner rientro su stream, Safe riconciliazione ref safe_tennis, scalper-service), `46a3f39`
FIX-A (Safe per modalità, Live P&L, /mike approva uscita, Omega tentativi non consumati). NON FATTO/DECISIONI: R8 Omega senza FOK in
paper (negato al delegato di B: decisione utente), «quote ferme non viste dai bot» (cancello decisionale: decisione utente),
canale_fase NULL sui terminali lato bot, EX_TRADED_VOL nello scanner, Match Replay in GBP, righe bot:<ref> riconciliazione.
**h19:07 — RIAVVIO 3 dell'app fatto da me** (CloseMainWindow → killChildren: 0 python/0 porte in 4 s; rilancio exe pid 22780): verifica
streaming/log/cambio in corso; poi B riaccende in paper; osservazione dai dati vivi; consolidato.
MIGRAZIONI DA APPLICARE (utente, in ordine): storico_esito_a_zero, omega_state_per_modalita, live_positions_senza_mercati_regolati,
analytics_rpc_veloci (PRIMA del job 03:23 UTC), betfair_live_orders_source_bot_2026-09-26; sanatorie SQL nei referti (origine='auto'
follow scalper di oggi; mike_trades None-None; proposte Safe > 12 h); pulizia kickoff analytics + refresh_analytics_riepilogo().
**(ora PC 19:10; NOTA: gli orari «h…» precedenti di questa sezione B sono ~40 min AVANTI rispetto all'orologio del PC; fanno fede gli UTC del DB e dei log)** — RIAVVIO 3: app riavviata da admin-26 alle 19:06:59 PC con chiusura ordinata (CloseMainWindow → killChildren: 0 python/0 porte in 4 s) e TUTTI i fix su master (suoi 67c3ad4, 0dbc127, 78d9f65, b0fe3b1, 07bc872, 73c1046, 2220288 = CustodeSessione nei runner; miei c4a5f30, 351121a, 46a3f39, 3900a2e). Streaming ok: `_logs/` 9 log dei figli (K2 risolto), cambio GBP→EUR 1,163 letto, alert 518 INFO «modalità PAPER attiva» (F-9 risolto), scanner source=stream 233 mercati, auto-follow 177. VIA dato al delegato per la riaccensione in paper; osservazione fino alle 19:40 PC; consolidato ~19:45 PC.
**(ora PC 19:30) RIAVVIO 3 acceso in PAPER (UTC)**: Omega 17:20:51, Mike 17:21:04, Safe 17:22:39 (4 varianti 6/6 paper; «pulsante assente» al primo clic = R-F2-17 ancora), tennis 17:22:53-17:23:03, scalper 17:23:20. 3° freno breve 17:27Z→~17:31Z. Riscontri: R-F2-C2 RISOLTO (alert 518); FIX-C dal vivo: due ripieghi REST da 31 s con RIENTRO sullo stream (alert 521-524), chiavi fonte/sessione/avvisi nello scanner_stato; R-F2-16 ANCORA APERTO (specchio source='runner' per Safe esatto dopo b0fe3b1: migrazione source_bot non applicata o ramo Safe fuori dal nuovo codice); **NUOVO R-F2-21 (verificato da me a DB)**: Mike scrive `loss_exit_deciso` in mike_activity 1 riga/s (712 righe 17:12:04-17:24:37Z sull'evento 36109477, 1,06 s/riga; 66 su 35925542), anche fra 17:12 e 17:21Z quando il bot era STOPPED (gestisce le posizioni aperte anche da fermo) → IO sul DB (cfr. caduta DB 13/09): KO aperto, priorità alta, file Betfair/mike/service.py (admin-26 oggi).
**(ora PC 19:35) RIAVVIO 3 chiuso** (referto in cima a `E2E_FASE2_BOT_PAPER_2026-09-26.md`, commit ab0808b): 15 controlli, 11 OK, 2 KO, 2 NC. 3° freno 17:27:09Z→17:31:14Z: scalper force-flat pulito, 0 armamenti a freno tirato (R-F2-10/11 non riprodotti), nessuna riga nuova; freno NON certificato sugli altri bot (nessun tentativo). Safe via canale+flumine, nessun doppio invio, nessun JOURNAL KO, scanner con chiavi nuove e 2 rientri sullo stream. Aperti: R-F2-16 (source='runner' finché non si applica `betfair_live_orders_source_bot`), R-F2-21 RIDOTTO ma non risolto da 8b0d04a (75 righe/5 min: la firma include cv_net/p4 che cambiano a ogni tick → segnalato ad admin-26), R-F2-6, R-F2-9, R-F2-17 (pulsante Safe assente al primo clic), F0 dai `_logs/` non analizzato.

### PUNTO DI RIPRESA (sessione B, fine 26/09, ora PC 19:35)
- **Stato**: app ACCESA (riavviata 19:06:59 PC) con master `ab0808b`; TUTTI i bot in PAPER (accesi 17:20-17:23Z): l'utente decide se lasciarli o spegnerli dalla Control Room; nessun ordine live oggi (0 righe live).
