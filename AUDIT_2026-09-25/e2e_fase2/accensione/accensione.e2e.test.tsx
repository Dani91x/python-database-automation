...
const FOTO = resolve(OUT, 'accensione');
...
describe('FASE 2 — accensione in paper', () => {
    ...
    it('A3 Safe → paper (base, esatto, punta; tennis dalla scheda tennis), strategy_modes 6/6 paper', async () => {
        ...
        await foto('A3_safe_dopo_base');
        for (const [sport, v] of [['calcio', 'esatto'], ['calcio', 'punta'], ['tennis', 'tennis']] as const) {
            await pausa(4000);
            s = await monta(sport);
    ...
    it('A4 4 bot tennis → paper', async () => {
        ...
        expect(righe.map((r) => [r.status, r.mode])).toEqual(Array(4).fill(['running', 'paper']));
        expect(await nessunLive()).toEqual([]);
    });
    ...
    it('A5 scalper → paper (strategia di default della UI = quella della riga)', async () => {
        ...
        expect([c.status, c.mode]).toEqual(['running', 'paper']);
        expect(await nessunLive()).toEqual([]);
    });
});
