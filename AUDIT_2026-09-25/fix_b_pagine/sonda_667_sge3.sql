select md5(public.get_market_delays(667, 'sge', '3', 'all', null, null)::text) as vecchia,
       md5((    with scope as (
    ...
    outcomes as (
        ...
        select s.fixture_id, s.fixture_date, s.home_team_name, s.away_team_name,
            ...
            when false and (s.hh is null or s.ha is null) then null  
              ...
              case 'sge'::text
                when 're'    then (s.h = null::integer and s.a = null::integer)::int
                when 'sge'   then ((s.h + s.a) = 3::integer)::int
    ...
    from derived d, scope_stats ss, over_under ou)::text) as nuova
