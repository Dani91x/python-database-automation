...
MUTAZIONI_PY = [
    ...
    # D3 (25/09): contratto nuovo, «live nasce in dry-run» (prima: live nasce live)
    ("live nasce con soldi veri (contratto di ieri)", SVC,
     '"dry_run": AM.dry_run_alla_nascita(modalita),', '"dry_run": modalita != "live",', T_PY),
    ("tutto nasce con soldi veri", SVC,
     '"dry_run": AM.dry_run_alla_nascita(modalita),', '"dry_run": False,', T_PY),
    ("dry-run di nascita spento", AM, "DRY_RUN_ALLA_NASCITA = True", "DRY_RUN_ALLA_NASCITA = False",
     T_PY),
    ("in live il dry-run blocca l'armamento", AM,
     '    if modalita == "live":\n        return None\n', "", T_PY),
