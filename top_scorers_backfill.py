...
# ========================
# API – fetch top scorers
# ========================


def fetch_top_scorers_from_api(
    league_id: int,
    season_year: int,
) -> Optional[Dict[str, Any]]:
    """
    Chiama /players/topscorers per league_id + season_year.
    Ritorna il JSON completo (dict) oppure None se errore/empty.
    """
    client = APIFootballClient()
    logger.info(
        "📡 Chiamata API /players/topscorers per league_id=%s, season_year=%s",
        league_id,
        season_year,
    )
    ...
    if not data:
        logger.warning(
            "⚠️ Nessuna risposta (data=None) da /players/topscorers per league_id=%s, season_year=%s",
            league_id,
            season_year,
        )
        return None
    ...
    logger.info("📌 Top scorers: elementi in 'response' = %s", len(resp_list))

    if not resp_list:
        logger.info(
            "📭 Nessun top scorer disponibile per league_id=%s, season_year=%s",
            league_id,
            season_year,
        )
        return None
...
def map_top_scorers_response_to_rows(
    data: Dict[str, Any],
    league_id: int,
    season_year: int,
) -> List[Dict[str, Any]]:
    """
    Mappa il JSON di /players/topscorers nella struttura della tabella public.top_scorers.
    ...
    resp_list = data.get("response") or []
    logger.info("📌 Numero top_scorers in 'response': %s", len(resp_list))
    ...
    logger.info("📌 Totale righe top_scorers mappate: %s", len(rows))
    return rows
...
def delete_existing_top_scorers(league_id: int, season_year: int) -> None:
    """
    Per idempotenza: cancella le righe esistenti in public.top_scorers
    ...
    logger.info(
        "🧹 Cancellazione top_scorers esistenti per league_id=%s, season_year=%s",
        league_id,
        season_year,
    )
    ...
    def _delete() -> None:
        ...
        logger.info(
            "   🗑️ Righe top_scorers cancellate per league_id=%s, season_year=%s: %s",
            league_id,
            season_year,
            deleted,
        )
    ...
    try:
        delete_con_ritentativi(
            _delete,
            etichetta=f"delete top_scorers league_id={league_id} season_year={season_year}",
        )
    except Exception as e:
        logger.error(
            "❌ Cancellazione top_scorers fallita dopo i ritentativi per league_id=%s, season_year=%s: %s",
            league_id,
            season_year,
            e,
        )
...
def insert_rows_top_scorers(rows: List[Dict[str, Any]], batch_size: int = 200) -> None:
    """
    Inserisce le righe nella tabella top_scorers, a chunk, con log verboso.
    """
    if not rows:
        logger.info("📭 Nessuna riga top_scorers da inserire.")
        return

    supabase = get_supabase()
    logger.info("📥 Insert top_scorers: %s righe", len(rows))
    start_time = time.time()

    for i in range(0, len(rows), batch_size):
        chunk = rows[i : i + batch_size]
        logger.info(
            "   🚚 Insert top_scorers batch %s (righe %s-%s)...",
            (i // batch_size) + 1,
            i + 1,
            i + len(chunk),
        )
        try:
            resp = supabase.table("top_scorers").insert(chunk).execute()
            data = getattr(resp, "data", None)
            logger.info(
                "   ✅ Insert top_scorers batch %s completato (righe inserite: %s)",
                (i // batch_size) + 1,
                len(data or []),
            )
        except Exception as e:
            logger.error(
                "   ❌ Errore insert top_scorers batch %s: %s",
                (i // batch_size) + 1,
                e,
            )

    elapsed_ms = int((time.time() - start_time) * 1000)
    logger.info("⏱️ Insert top_scorers completato in %sms", elapsed_ms)


# ========================
# Orchestratore top_scorers
# ========================


def backfill_top_scorers_for_league_season(
    league_id: int,
    season_year: int,
) -> None:
    """
    Orchestratore:
      - chiama /players/topscorers
      - mappa il JSON in righe
      - cancella i top_scorers esistenti per (league_id, season_year)
      - inserisce le nuove righe
    """
    logger.info(
        "🚀 Inizio backfill top_scorers per league_id=%s, season_year=%s",
        league_id,
        season_year,
    )

    data = fetch_top_scorers_from_api(league_id, season_year)
    if not data:
        logger.warning(
            "⚠️ Nessun dato top_scorers da processare per league_id=%s, season_year=%s",
            league_id,
            season_year,
        )
        return

    rows = map_top_scorers_response_to_rows(data, league_id, season_year)
    if not rows:
        logger.warning(
            "⚠️ Nessuna riga top_scorers mappata per league_id=%s, season_year=%s",
            league_id,
            season_year,
        )
        return

    delete_existing_top_scorers(league_id, season_year)
    insert_rows_top_scorers(rows)

    logger.info(
        "🏁 Backfill top_scorers completato per league_id=%s, season_year=%s (righe inserite: %s)",
        league_id,
        season_year,
        len(rows),
    )
...
def ask_and_run_cli():
    print("==============================================")
    print("  🎯 Backfill top_scorers da API-FOOTBALL")
    print("==============================================")
